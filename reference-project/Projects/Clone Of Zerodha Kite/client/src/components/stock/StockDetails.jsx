    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            {quote.simulated && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-kite-yellow/20 text-kite-yellow">
                Simulated
              </span>
            )}
          </div>
        ...
        <div className={`p-2.5 rounded-xl ${isPositive ? 'bg-kite-green/10' : 'bg-kite-red/10'}`}>
          {isPositive ? (
            <TrendingUp className="w-6 h-6 text-kite-green" />
          ) : (
            <TrendingDown className="w-6 h-6 text-kite-red" />
          )}
        </div>
      </div>
      ...
      <div className="flex items-baseline gap-3 mb-5">
        ...
        <div className={`flex items-center gap-1 ${isPositive ? 'price-up' : 'price-down'}`}>
          ...
          <span className={`text-sm font-mono px-2 py-0.5 rounded-full ${
            isPositive ? 'bg-kite-green/15' : 'bg-kite-red/15'
          }`}>
      ...
      {quote.low && quote.high && (
        <div className="mt-4 pt-4 border-t border-kite-border/30">
          <div className="flex justify-between text-xs text-kite-muted mb-1">
            <span>Day Low</span>
            <span>Day High</span>
          </div>
