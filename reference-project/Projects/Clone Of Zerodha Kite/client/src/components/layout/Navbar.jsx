...
import {
  ...
  Bell, LogOut, User, Wifi, WifiOff, Menu, X,
} from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-kite-surface/95 backdrop-blur-md border-b border-kite-border/50">
      <div className="max-w-[1600px] mx-auto px-4">
        <div className="flex items-center justify-between h-14">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-kite-blue to-kite-accent flex items-center justify-center">
              <TrendingUp className="w-5 h-5 text-white" />
            </div>
            <span className="text-lg font-bold bg-gradient-to-r from-kite-blue to-kite-accent bg-clip-text text-transparent">
              KiteX
            </span>
          </Link>
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    active ? 'text-kite-blue' : 'text-kite-muted hover:text-kite-text'
                  }`}
                >
                  ...
                  {active && (
                    <motion.div
                      layoutId="nav-active"
                      className="absolute inset-0 bg-kite-blue/10 rounded-lg border border-kite-blue/20"
